	...
	function checkscala() {
		...
		if(carte[get_Hid(cartatmp)].id==1) {
			cartatmp=$(cartatmp).prev(); var cont=2;
			while(exist(cartatmp) && !carte[get_Hid(cartatmp)].ishidden && cont<=13) {
				if(carte[get_Hid(cartatmp)].id!=cont) return;
	...
	function numtosrc(carta) {
		var percorso="carte/picche/"; var ext=".svg";
	...
	function loadEvents() {
		...
		$(".cont_carta").on('mousedown',function(){
			...
			if(!carte[get_Hid(cartamossa)].ishidden){
				...
				$(altrecarte).each(function() { if(carte[get_Hid(this)].id != carte[get_Hid($(this).prev())].id-1) blocco=true; });
				if(!blocco) move=true;
			}
		});
	...
	function start() {
		dim=new dimension();
		carte= new Array();
		for(var i=0;i<numcarte;i++) carte[i]= new carta(genera_id(i+1));
		mescola();
